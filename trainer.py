...
def update_replay_buffer(
    ...
    for timestep in range(len(episode_experience)):
        ...
        if her_type == HERType.FINAL:
            ...
            # compute new reward
            new_goal = _next_state
            new_reward = -torch.dist(torch.tensor(state), torch.tensor(new_goal))

            # new conditioned state
            new_state = np.concatenate((state, new_goal), axis = 0)
            new_goal = np.concatenate((next_state, new_goal), axis = 0)

            # add to buffer
            replay_buffer.add(new_state, action, new_reward, new_goal)
            ...
            i = np.random.randint(timestep, len(episode_experience))
            _state, _action, _reward, _next_state, _goal = episode_experience[i]
            ...
            # get random goal
            i = np.random.randint(0, len(episode_experience))
            _state, _action, _reward, _next_state, _goal = episode_experience[i]
...
def train(
    ...
    #print(q_network.dtype)
    model = q_network.QNetwork(input_dim, action_dim)
    #print(model.dtype)
    #if 'SawyerActionDiscretize' in str(env):
    #    model = model.double()
    ...
    for epoch_idx in range(num_epochs):
        ...
        for _ in range(opt_steps):
            ...
            ###
            #print(state.dtype)   # torch.float32
            #print(model_predict.dtype) # torch.float32
            ###
